...
1. create a config json file at `script/templates/markets/{LOAN_TOKEN_NAME_COLLATERAL_TOKEN_NAME}.json`. for example:
```json
{
...
1. create a config json file at `script/templates/vaults/{VAULT_NAME}.json`. for example:
```json
{
...
This script can only be called while we are the vault curator; we will be setting the final curator at the end of this process
...
This must be done immediately after deployment of the market to prevent 0% utilization decay ([see morpho docs](https://docs.morpho.org/curate/tutorials-market-v1/creating-market/#fill-all-attributes))

Assuming `NEW_MARKET_PATH` is still set in the env

Assuming the config json contains positive values for `vaultDepositAssets`, `collateralAmount`, `borrowAssets` (in decimals) AND the sender has sufficient balance of the collateral token
...
This will transfer curator role to the final curator address and cannot be undone; this script should only be run _after_ the previous step
